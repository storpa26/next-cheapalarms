  ...
  for (let i = 0; i < count; i++) {
    ...
    jobs.push({
      ...
      scheduledEndDate: scheduledDate
        ? new Date(scheduledDate.getTime() + 2 * 60 * 60 * 1000).toISOString()
        : null,
      startedAt,
      completedAt,
...
export function generateServiceM8Workers() {
  ...
  for (let i = 0; i < offlineCount; i++) {
    const worker = getRandomItem(
      workerNames.filter((w) => !workers.find((wo) => wo.name === w))
    );
    if (!worker) break;
